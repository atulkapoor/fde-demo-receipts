...
The engine wired in below (`scan`) is the second half of that pipeline only.
This corpus hands the build the scanner's own character output, so the pixel
half is already done: what is left is lines, a legibility score per line, and
the label/value pairing a receipt's two-column tail needs once a scan has
flattened it into one column. Swapping in tesseract -- the alternative the
registry lists -- replaces the reading, not this pairing.

Regions are labelled by layout role (header, body, value) and never by field
...
from app.shapes import documents_of, require
...
CURRENCY = {"RM", "MYR", "$", "USD"}
# The GST codes a receipt prints against each row. Like the currency marker,
# a line made of nothing but one of these names nothing -- and read as a
# label it shifts every pairing below it by one, which is how the amount
# beside "final total" ends up being the change. A single-letter code is
# already excluded by length; these are the ones long enough to read as a
# word.
TAX_CODES = {"SR", "ZR", "ZRL", "OS", "ES", "NR", "TX", "SV"}
TOKENS = re.compile(r"[\s:]+")


def pages_of(payload: dict[str, Any]) -> list[Any]:
    """The scanned pages, however the caller supplied them.

    `pages` is the contract's own name for them. A caller who sent one
    scan as text reaches here as a one-page document (app/shapes.py builds
    it), so that is read too -- and a request carrying neither is refused
    by name rather than answered from nothing.
    """
    if "pages" in payload:
        return require(payload, "pages", list, non_empty=True)
    if "documents" in payload:
        return documents_of(payload)
    raise RefusedInput("missing 'pages'")
    ...
    def run(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        for position, page in enumerate(source):
            ...
            if not isinstance(page.get("text", ""), str):
                raise RefusedInput(f"pages[{position}] needs a string 'text'")
            regions = self._engine(page)
            if not regions:
                # A page nothing could be read from. Refused here rather than
                # carried forward as an empty record: four nulls reported as
                # an extraction is how a queue of things worth checking turns
                # into a page of confident blanks.
                raise RefusedInput(f"pages[{position}] holds no legible text")
        ...
        return {
            **payload,
            "pages": pages,
            # The record every later step reads: one per page. `raw` carries
            # the regions as the engine read them, because the mapper's
            # candidates have to be lines off this page and nothing else.
            "records": [{"id": p["id"], "text": p["text"],
                         "raw": {"regions": p["regions"]},
...
def _pair(labels: list, values: list) -> None:
    ...
    for reading in (zip(labels, values, strict=False),
                    zip(labels[-width:], values[-width:], strict=False)):
...
def _pair_columns(regions: list[dict[str, Any]]) -> None:
    ...
    """
    runs = _runs(regions)
    for (kind, run), (next_kind, next_run) in zip(runs, runs[1:], strict=False):
